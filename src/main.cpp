...
// const int pumpPin = PA15;  // Pump 8
// const int sensorPin = PA7; // Sensor for Pump 8
...
//   Serial.println("✅ Pump 8 Test System Ready");
//   Serial.println("📘 Commands:");
//   Serial.println("  D8:<volume>       - Dispense volume (mL)");
//   Serial.println("  D8:<v>:<cal>@<r> - Set cal & dispense (e.g. D4:100:11.5@1)");
//   Serial.println("  D8:<r>:<cal>      - Set calibration only (e.g. D4:2:30)");
...
// this is for the time based testing of the pump 8
#include <Arduino.h>

const int pumpPin = PA15;
...
void loop() {
  if (Serial.available()) {
    ...
    if (cmd == "D8") {
      ...
      startTime = millis();
      pumpRunning = true;
      Serial.println("⏱️ Pump ON");
    }

    else if (cmd == "S" && pumpRunning) {
      digitalWrite(pumpPin, LOW);
      unsigned long duration = millis() - startTime;
      Serial.print("⏹️ Pump OFF - Total time: ");
